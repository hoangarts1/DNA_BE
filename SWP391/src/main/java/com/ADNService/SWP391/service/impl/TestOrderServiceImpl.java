...
public class TestOrderServiceImpl implements TestOrderService {
    ...
    private void generateFormDanSu(Document document, TestOrder order) {
        generateCommonFirstPage(document, order, false);
        generateCommonSecondPage(document);
    }
    ...
    private void generateFormHanhChinh(Document document, TestOrder order) {
        ...
        generateCommonFirstPage(document, order, true);
        generateCommonSecondPage(document);
        ...
        thuMauCellFinal.add(new Paragraph(getNormalStaffName(order))
                .setFontSize(12)
                .setTextAlignment(TextAlignment.CENTER)
...
// Cột NGƯỜI YÊU CẦU XÉT NGHIỆM
    ...
    private void generateCommonFirstPage(Document document, TestOrder order, boolean isHanhChinh) {
        Customer customer = order.getCustomer();
        Account acc = customer.getAccount();
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
        ...
        DeviceRgb blueColor = new DeviceRgb(0x00, 0x4E, 0xB2);
        DeviceRgb orangeColor = new DeviceRgb(0xF6, 0x94, 0x1C);

        String[] headers = {"STT", "Họ tên", "Năm sinh", "Giới tính", "Mối quan hệ", "Loại mẫu"};
        for (String header : headers) {
            sampleTable.addHeaderCell(new Cell()
        ...
        // Fingerprint (only for Hanh Chinh)
        ...
        requesterCell.add(new Paragraph(fullName)
                .setFontSize(12)
                .setMarginTop(70.25f));
    ...
    private void generateCommonSecondPage(Document document) {
        document.getPdfDocument().addNewPage();
        document.setMargins(20, 20, 20, 20);
